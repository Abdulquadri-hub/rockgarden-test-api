...
class IncidentEventListener
{
     ...
     * Handle the event.
     *
     * @param  \App\Events\IncidentEvent  $event
     * @return void
     */
    public function handle(IncidentEvent $event)
    {
        ...
        try {
            ...
            if($dto->getType() == EventType::INCIDENT){
                if(!empty($contact)){
                    //Send Email
                    if (filter_var($contact['email'], FILTER_VALIDATE_EMAIL) && $send_email) {
                        Mail::to($contact['email'])->send(new NewIncidentAdminMail($dto->getClientFullname(), $dto->getDashboardLink()));
                    }

                    // Send Sms to user
                    if(!empty($contact['phone']) && $send_sms){
                        TwilioSMSController::sendSMS($contact['phone'], TwilioSMSController::newIncidentAdminMessage($dto->getClientFullname(), $dto->getDashboardLink()));
                    }
                }
            }
